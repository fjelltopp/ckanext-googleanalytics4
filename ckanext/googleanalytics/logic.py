...
class AnalyticsPostThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            measurement_id = toolkit.config.get(
                "googleanalytics.measurement_id"
            )
            api_secret = toolkit.config.get("googleanalytics.api_secret")
            res = requests.post(
                "https://www.google-analytics.com/mp/collect?measurement_id={}&api_secret={}".format(
...
def post_analytics(user):
    ...
    resource_data = {
        "client_id": hashlib.md5(
            six.ensure_binary(toolkit.c.user)
        ).hexdigest(),
        "events": [
            {"name": "file_download", "params": {"link_url": referer_link}}
        ],
    }
